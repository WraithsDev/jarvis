...
from actions.windows_utils import play_mp3_process

BASE_DIR = Path(__file__).resolve().parent

SYSTEM_NAME = "J.A.R.V.I.S"
SYSTEM_TAGLINE = "Realtime Assistant"
MODEL_BADGE = "VOICE CORE / WINDOWS"
BUILD_BADGE = "LOCAL SYSTEM"
...
# Orb durum renkleri
ORB_COLORS = {
    "LISTENING":    (77, 163, 255),
    "SPEAKING":     (125, 207, 255),
    "THINKING":     (120, 170, 255),
    "MUTED":        (200, 30, 80),
    "PAUSED":       (30, 55, 85),
    "ERROR":        (255, 51, 68),
...
# Grift fontu kullanÄ±cÄ±nÄ±n sisteminde yÃ¼klÃ¼. Basliklarda daha sert bir vurgu
...
class SoundManager:
    ...
    def _start_sound(self, path: Path, volume: float):
        proc = play_mp3_process(path, volume)
        with self._lock:
    ...
    def set_enabled(self, enabled: bool):
        ...
        if enabled:
            # Keep HUD.mp3 disabled. It is a looping ambience and can keep
            # playing after crashes; one-shot effects are safer.
            self._stop_ambient()
        else:
            self._stop_ambient()
            self._stop_foreground()
...
class JarvisUI:
    def __init__(self):
        ...
        self.W = min(W_TARGET, max(820, sw - 80))
        self.H = min(H_TARGET, max(560, sh - 110))
        pos_x = max(12, sw - self.W - 28)
        pos_y = max(12, sh - self.H - 72)
        ...
        # BirkaÃ§ saniye sonra topmost'u kapat (normal davranÄ±ÅŸ)
        ...
        self.tick            = 0
        self.rings_spin      = [0.0, 45.0, 90.0, 200.0]  # 4 ayrÄ± halka
        self.pulse_r         = []
        ...
        self._weather_card = {
            "city": "Istanbul",
            "primary": "--",
            "details": ["Weather data loading..."],
        }
        self._health_card_lines = ["System summary loading..."]
        self._panel_focus = ""
        ...
        # â”€â”€ PartikÃ¼ller (arka plan, az sayÄ±da) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        self.particles = [
            {
        ...
        self.orb_particles = [
            {
                ...
                'speed': random.uniform(-0.038, 0.038),
        ...
        self.orb_shell_particles = [
            {
                'angle': random.uniform(0, math.tau),
                'speed': random.uniform(-0.026, 0.026),
                'size': random.uniform(1.2, 4.4),
                'phase': random.uniform(0, math.tau),
    ...
    def _enter_assistant_window(self):
        sw = self.root.winfo_screenwidth()
        sh = self.root.winfo_screenheight()
        width, height = self._normal_size
        pos_x = max(12, sw - width - 28)
        pos_y = max(12, sh - height - 72)
        self._window_geometry = f"{width}x{height}+{pos_x}+{pos_y}"
    ...
    def _set_layout_metrics(self, width: int, height: int):
        ...
        self.FACE = min(int(orb_area_h * 0.96), int(center_w * 0.76), 980)
    ...
    # â”€â”€ Shutdown button (saÄŸ alt, bÃ¼yÃ¼k) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _draw_shutdown_button(self):
        ...
        c.create_text(BW//2, BH//2, text="BENI KAPAT",
    ...
    def _build_settings_panel(self):
        ...
        self._settings_body = tk.Frame(self._settings_panel, bg="#06101d")
        self._debug_body = tk.Frame(self._settings_panel, bg="#06101d")
    ...
    def _draw_settings_button(self):
        ...
        inner = "#0b1f38" if self._settings_open else "#06101d"
        ...
        c.create_text(bw - 14, bh // 2, text="OPEN" if self._settings_open else "MENU",
    ...
    def _draw_settings_tabs(self):
        for key, canvas, label in (
            ...
            fill = "#0b2340" if active else "#06101d"
            text_col = C_PRI if active else "#6f96c6"
    ...
    def _refresh_settings_status(self):
        ...
        primary = [
            "Gemini ready" if gemini_ready else "Gemini key missing",
            "YouTube ready" if yt_key_ready and yt_handle else "YouTube optional",
        ]
        if yt_handle:
    ...
    def _render_debug_logs(self):
        ...
        if not self._debug_entries:
            self._debug_text.insert(tk.END, "No diagnostics yet.\n", "info")
        else:
    ...
    def _sync_sound_state(self):
        enabled = bool(getattr(self, "_sfx_on", False))
    ...
    def _draw_mute_button(self):
        ...
        c.delete("all")
        if self.muted:
            col, icon, lbl = C_MUTED, "", "SUSTUR"
        else:
            col, icon, lbl = C_GREEN, "", "DINLIYOR"
        bl = 6
        for bx, by, sx, sy in [(0, 0, 1, 1), (bw, 0, -1, 1),
                                (0, bh, 1, -1), (bw, bh, -1, -1)]:
    ...
    # â”€â”€ Orb tÄ±klama = pause â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _toggle_pause(self):
        ...
        if self.paused:
            ...
            self.write_log("SYS: JARVIS resumed.")
        self._sync_sound_state()
    ...
    def _build_input_bar(self, lw: int):
        ...
        self._input_entry = tk.Entry(
            ...
            fg=C_TEXT, bg="#0a1218", insertbackground=C_TEXT,
        ...
        self._send_btn = tk.Button(
            ...
            fg="#101820", bg=C_ORG,
            activeforeground="#101820", activebackground=C_ORG2,
            font=font_body_bold(10),
    ...
    def _on_input_submit(self, event=None):
        ...
        if not text:
            return
        if self.paused:
            self.write_log("SYS: JARVIS is paused. Press resume to continue.")
            return
        self._input_var.set("")
        if text.lower() in ("sus", "dur", "stop", "sessiz", "kes"):
            self.write_log("SYS: Audio stopped.")
    ...
    def set_state(self, state: str):
        ...
        elif state == "ERROR" and previous != "ERROR":
    ...
    def _parse_weather_card(self, text: str) -> dict:
        if not text or "alinamadi" in text.lower() or "unavailable" in text.lower():
            return {
                "city": "Istanbul",
                "primary": "--",
        ...
        city = "Istanbul"
        if " icin" in prefix:
            city = prefix.split(" icin", 1)[0].strip().title()
    ...
    def _parse_health_card(self, text: str) -> list[str]:
        ...
        return lines or ["System summary not ready."]
    ...
    def _bar(self, c, x, y, w, h, pct, color):
        c.create_rectangle(x, y, x+w, y+h, fill="#071323", outline=C_DIM, width=1)
        fw = max(1, int(w * pct / 100))
        c.create_rectangle(x+1, y+1, x+fw, y+h-1, fill=color, outline="")

    def _sparkline(self, c, x, y, w, h, data):
        c.create_rectangle(x, y, x+w, y+h, fill="#050d19", outline=C_DIM, width=1)
        n = len(data)
        if n < 2:
            return
        step = (w - 2) / (n - 1)
    ...
    def _draw_info_card(self, c, x0, y0, pw, ph, title, accent=C_PRI):
        ...
        border = accent if focus > 0.08 else ("#314d72" if dimmed else self._ac(45, 110, 190, 190))
        fill = "#08111f" if dimmed else "#050d19"
        ...
        line_fill = "#173255" if dimmed else C_DIM
    ...
    def _draw_health_overlay(self, c):
        ...
        c.create_text(x0+pw//2, y0+18, text="SYSTEM HEALTH",
    ...
    # â”€â”€ SaÄŸ panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def _draw_right_panel(self, c):
        ...
        c.create_rectangle(x0, y0, x0+pw, y0+ph, fill="#07101d", outline="")
        self._bracket(c, x0, y0, pw, ph, col="#28598f")
    ...
    # â”€â”€ ORB (ana Ã§izim) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def _draw_orb(self, c):
        state = "PAUSED" if self.paused else self._jarvis_state
        t = self.tick
        R, G, B = self._orb_rgb()
        activity = 0.10 if self.paused else 1.0 if self.speaking else 0.82 if self.user_speaking else 0.58 if state in ("THINKING", "INITIALISING") else 0.34
        if state in ("THINKING", "INITIALISING"):
            accent_rgb = (130, 178, 255)
        elif self.speaking:
            accent_rgb = (150, 225, 255)
        elif self.user_speaking:
            accent_rgb = (105, 190, 255)
        else:
            accent_rgb = (85, 170, 255)

        cx = self.FCX
        cy = self.FCY + int(4 * math.sin(t * 0.028))
        base = int(self.FACE * (0.32 + 0.030 * activity))
        pulse = 1.0 + (0.025 + 0.032 * activity) * math.sin(t * (0.050 + 0.020 * activity))
        radius = int(base * pulse)
        halo = max(22, int(self.halo_a * (0.60 + activity)))

        # Cinematic volumetric glow.
        for i in range(8, 0, -1):
            rr = int(radius * (0.92 + i * 0.095))
            alpha = int(halo * (0.018 + i * 0.010))
            c.create_oval(cx-rr, cy-rr, cx+rr, cy+rr, outline=self._ac(R, G, B, alpha), width=2)

        # Dark inner mass gives the blue energy a film-like contrast.
        for i in range(7, 0, -1):
            rr = int(radius * (0.18 + i * 0.095))
            shade = 10 + i * 5
            c.create_oval(cx-rr, cy-rr, cx+rr, cy+rr, fill=self._ac(4, 12 + shade, 28 + shade, 22), outline="")

        # Reactor shell with offset orbital planes.
        rings = (
            (1.16, self.rings_spin[0], 96, 4, (R, G, B), 190),
            (1.16, self.rings_spin[0] + 188, 54, 3, accent_rgb, 145),
            (0.96, -self.rings_spin[1] + 24, 74, 3, (105, 195, 255), 138),
            (0.78, self.rings_spin[2] + 214, 112, 2, accent_rgb, 112),
            (0.58, -self.rings_spin[3] + 42, 80, 2, (R, G, B), 90),
        )
        for frac, start, extent, width, color, alpha in rings:
            rr = int(radius * frac)
            c.create_arc(
                cx-rr, cy-rr, cx+rr, cy+rr,
                start=start % 360,
                extent=extent + int(22 * activity),
                outline=self._ac(color[0], color[1], color[2], alpha),
                width=width,
                style="arc",
            )

        # Energy filaments crossing the core.
        filament_count = 9
        for i in range(filament_count):
            a = (math.tau * i / filament_count) + t * (0.010 + 0.006 * activity)
            bend = 0.25 * math.sin(t * 0.035 + i)
            r1 = radius * (0.18 + 0.03 * math.sin(t * 0.04 + i))
            r2 = radius * (0.86 + 0.05 * math.cos(t * 0.025 + i))
            x1 = cx + math.cos(a + bend) * r1
            y1 = cy + math.sin(a + bend) * r1 * 0.72
            x2 = cx + math.cos(a + math.pi - bend) * r2
            y2 = cy + math.sin(a + math.pi - bend) * r2 * 0.72
            c.create_line(x1, y1, x2, y2, fill=self._ac(accent_rgb[0], accent_rgb[1], accent_rgb[2], 34 + int(52 * activity)), width=1, smooth=True)

        # Inner waveform ring. Smooth breathing plasma.
        points = []
        count = 64
        wave_r = radius * 0.42
        amp = radius * (0.028 + 0.040 * activity)
        for i in range(count + 1):
            a = math.tau * i / count
            wobble = math.sin(a * 4 + t * 0.070) + 0.55 * math.sin(a * 7 - t * 0.045)
            rr = wave_r + amp * wobble
            points.extend([cx + math.cos(a) * rr, cy + math.sin(a) * rr])
        c.create_line(*points, fill=self._ac(accent_rgb[0], accent_rgb[1], accent_rgb[2], 160), width=2, smooth=True)

        # Particle halo, reusing seeded particles so motion is stable and fluid.
        for i, p in enumerate(self.orb_shell_particles[:26]):
            a = p["angle"] + t * p["speed"] * (0.55 + activity)
            rr = radius * (0.70 + 0.30 * p["glow"] + 0.030 * math.sin(t * 0.035 + p["phase"]))
            x = cx + math.cos(a) * rr
            y = cy + math.sin(a) * rr * (0.72 + 0.10 * math.sin(a + t * 0.015))
            depth = 0.45 + 0.55 * (1 + math.sin(a + t * 0.020)) / 2
            sr = p["size"] * (0.65 + activity * 0.45) * depth
            alpha = int((62 + 92 * p["glow"]) * depth)
            color = accent_rgb if i % 4 == 0 else (R, G, B)
            c.create_oval(x-sr, y-sr, x+sr, y+sr, fill=self._ac(color[0], color[1], color[2], alpha), outline="")

        # Expanding pulse rings for voice/activity.
        for pr in self.pulse_r:
            rr = int(radius * 0.28 + pr)
            if rr <= radius * 1.22:
                alpha = max(0, int(110 * (1 - rr / (radius * 1.24))))
                c.create_oval(cx-rr, cy-rr, cx+rr, cy+rr, outline=self._ac(R, G, B, alpha), width=1)

        # Center core.
        for i in range(6, 0, -1):
            rr = int(radius * (0.045 + i * 0.025))
            c.create_oval(cx-rr, cy-rr, cx+rr, cy+rr, fill=self._ac(R, G, B, 12 + i * 18), outline="")
        core = int(radius * (0.040 + 0.010 * math.sin(t * 0.09)))
        c.create_oval(cx-core, cy-core, cx+core, cy+core, fill=self._ac(210, 238, 255, 220), outline="")

    # â”€â”€ Ana Ã§izim â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def _draw(self):
        c  = self.bg
        W  = self.W
        H  = self.H
        ...
        # Nokta Ä±zgarasÄ± â€” Ã§ok ince
        step = 132
        for x in range(0, W, step):
            for y in range(0, H, step):
                c.create_rectangle(x, y, x+1, y+1, fill=C_DIMMER, outline="")

        # Tarama Ã§izgisi (yavaÅŸ, Ã§ok soluk)
        scan_y = (t * 0.7) % (H + 60) - 30
        for i in range(2):
            ly = (scan_y + i * 20) % H
            c.create_line(0, ly, W, ly+35, fill="#07172b", width=1)

        # PartikÃ¼ller
        R, G, B = self._orb_rgb()
        for p in self.particles[:10]:
            if self.speaking:
                col = self._ac(255, 110, 0, int(p['a'] * 0.45))
            else:
                col = self._ac(R, G, B, int(p['a'] * 0.45))
            r = p['r']
            c.create_oval(p['x']-r, p['y']-r, p['x']+r, p['y']+r,
                          fill=col, outline="")

        # â”€â”€ BÃ¶lÃ¼cÃ¼ Ã§izgiler (ince, soluk) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        c.create_text(self.FCX, self.CTRL_Y - 12, text=f"STATUS / {state_label.title()}",
                      fill=state_col, font=font_body_bold(11))

        # â”€â”€ HEADER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        c.create_rectangle(0, 0, W, HDR_H, fill="#050a16", outline="")
        c.create_line(0, HDR_H, W, HDR_H, fill=C_MID, width=1)
        for i in range(3):
            a = 60 - i * 18
            c.create_line(0, HDR_H-1-i, W, HDR_H-1-i,
                          fill=self._ac(77, 163, 255, a), width=1)

        c.create_text(W//2, 24, text=SYSTEM_NAME,
                      fill="#edf7ff", font=font_display(26))
        c.create_text(W//2, 52, text=SYSTEM_TAGLINE,
                      fill=C_MID, font=font_body(11))

        c.create_text(22, 36, text="VOICE CORE",
                      fill="#2b5b92", font=font_body(10), anchor="w")

        # SaÄŸ: durum indikatÃ¶rÃ¼
    ...
    def _show_setup_ui(self, edit_mode: bool = False):
        ...
        self.setup_frame = tk.Frame(self.root, bg="#030914",
        ...
        title = "API SETTINGS" if edit_mode else "FIRST RUN SETUP"
        subtitle = (
            "Update Gemini and optional YouTube settings."
            if edit_mode else
            "Enter your Gemini API key. YouTube fields are optional."
        )
        config = load_app_config()

        tk.Label(self.setup_frame, text=title,
                 fg=C_PRI, bg="#030914", font=font_display(20)).pack(pady=(28, 6))
        tk.Label(self.setup_frame, text=subtitle,
                 fg=C_MID, bg="#030914", font=font_body(13)).pack(pady=(0, 14))
        tk.Label(self.setup_frame, text="GEMINI API KEY",
                 fg=C_DIM, bg="#030914", font=font_body(12)).pack(pady=(8, 4))

        self.api_entry = tk.Entry(
            self.setup_frame, width=60,
            fg=C_TEXT, bg="#071426", insertbackground=C_TEXT,
        ...
        tk.Label(self.setup_frame, text="YOUTUBE API KEY",
                 fg=C_DIM, bg="#030914", font=font_body(12)).pack(pady=(10, 4))
        ...
        buttons = tk.Frame(self.setup_frame, bg="#030914")
        buttons.pack(pady=14)

        tk.Button(buttons, text="SAVE",
                  command=self._save_api_key, bg=C_BG, fg=C_PRI,
                  activebackground="#0b3a66", font=font_body_bold(13),
                  borderwidth=0, padx=24, pady=10).pack(side="left", padx=8)

        if edit_mode:
            tk.Button(buttons, text="CLOSE",
                      command=self._close_setup_ui, bg="#08111f", fg=C_DIM,
                      activebackground="#10233a", font=font_body_bold(13),
                      borderwidth=0, padx=24, pady=10).pack(side="left", padx=8)
