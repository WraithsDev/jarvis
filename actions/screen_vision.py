...
HELPER_BIN = HELPER_BIN_DIR / "jarvis-screen-helper"
...
def _screen_permission_message() -> str:
    if sys.platform.startswith("win"):
        return (
            "Ekran goruntusu alinamadi. Windows'ta ekran analizi icin pyautogui ve Pillow "
            "kurulu olmali; uygulama uzak/korumali bir oturumda calisiyorsa ekran yakalama engellenebilir."
        )
    return (
        "Ekran analizi icin Windows ekran kaydi izni gerekiyor. "
...
def _run_helper(mode: str, timeout: int = 20) -> tuple[bool, str]:
    if sys.platform.startswith("win"):
        if mode != "capture_active_window":
            return False, "Desteklenmeyen ekran yakalama modu."
        try:
            import pyautogui
            ...
            return True, json.dumps(
                {
                    ...
                    "bounds": {},
                    "detail": "windows_pyautogui",
                },
                ensure_ascii=False,
            )
        except Exception as exc:
            return False, f"Windows ekran goruntusu alinamadi: {exc}"
