"""
WhatsApp mesaj gonderme - Windows'ta WhatsApp Desktop veya Web uzerinden calisir.

Desteklenen akÄ±ÅŸlar:
- WhatsApp Desktop URL scheme ile numaraya sohbet aÃ§ma
- WhatsApp Desktop iÃ§inde kiÅŸi adÄ±na gÃ¶re sohbet arama
- WhatsApp Web Ã¼zerinden telefon numarasÄ±yla taslak aÃ§ma
- SÄ±k kullanÄ±lan kiÅŸileri kalÄ±cÄ± belleÄŸe kaydetme

Not:
- Otomatik gonderim Windows'ta aktif pencereye Enter gondererek best-effort calisir.
"""
...
from actions.windows_utils import copy_to_clipboard, open_url, press_enter_after_delay
...
def _normalize_phone(phone_number: str) -> str:
    ...
    if len(digits) < 8 or len(digits) > 15:
        raise ValueError(
            "Telefon numarasÄ± uluslararasÄ± formatta olmalÄ±. "
            "Ornek format: +<ulke_kodu><numara>"
        )
    return digits
...
def save_whatsapp_contact(display_name: str, phone_number: str, aliases: str = "") -> str:
    ...
    if alias_list:
        return f"{display_name.strip()} WhatsApp kiÅŸilerine kaydedildi. Takma adlar: {', '.join(alias_list)}"
    return f"{display_name.strip()} WhatsApp kiÅŸilerine kaydedildi."
...
def import_phone_book_from_vcf(vcf_path: str) -> str:
    ...
    if not source.exists():
        return f"Rehber dosyasÄ± bulunamadÄ±: {source}"

    try:
        text = source.read_text(encoding="utf-8", errors="ignore")
    except Exception as exc:
        return f"Rehber dosyasÄ± okunamadÄ±: {exc}"
    ...
    return f"{imported} rehber kiÅŸisi iÃ§e aktarÄ±ldÄ±, {skipped} kayÄ±t atlandÄ±."
...
def _unsupported_desktop_script(script: str, timeout: int = 18) -> tuple[bool, str]:
    return False, "Bu otomasyon Windows surumunde kullanilmiyor."
...
def _auto_send_with_enter(app_name: str) -> tuple[bool, str]:
...
def _open_whatsapp_desktop_via_scheme(phone_number: str, message: str) -> tuple[bool, str]:
    ...
    try:
        ...
        return False, f"WhatsApp Desktop aÃ§Ä±lamadÄ±: {exc}"


    return True, "WhatsApp Desktop sohbeti aÃ§Ä±ldÄ±."


def _open_whatsapp_desktop_by_name(recipient_name: str, message: str, send_now: bool) -> tuple[bool, str]:
    return False, "Windows'ta kisi adiyla otomatik WhatsApp Desktop aramasi desteklenmiyor; kayitli telefon numarasi veya WhatsApp Web kullaniliyor."
...
def _open_whatsapp_web(phone_number: str, message: str) -> tuple[bool, str]:
    ...
    try:
        ...
        return False, f"WhatsApp Web aÃ§Ä±lamadÄ±: {exc}"
...
def send_whatsapp_message(
    ...
    if resolved_name and normalized_phone and (contact is None or contact.get("_source") == "phone_book"):
        # Bir kullanÄ±cÄ± numarayla birlikte isim verdiyse bunu sÄ±k kullanÄ±lanlara ekle.
    ...
    if app_target in {"auto", "desktop"}:
        if normalized_phone:
            ...
            if ok:
                ...
                if not send_now:
                    ...
                    return f"WhatsApp Desktop iÃ§inde {label}{source_note} iÃ§in taslak mesaj aÃ§Ä±ldÄ±."
                ok_send, send_detail = _auto_send_with_enter("WhatsApp")
                if ok_send:
                    label = resolved_name or f"+{normalized_phone}"
                    return f"WhatsApp Desktop Ã¼zerinden {label}{source_note} kiÅŸisine mesaj gÃ¶nderildi."
                return (
                    "WhatsApp Desktop sohbeti aÃ§Ä±ldÄ± ama otomatik gÃ¶nderim tamamlanamadÄ±. "
                    f"{send_detail}. Sistem AyarlarÄ± > Gizlilik ve GÃ¼venlik > EriÅŸilebilirlik izni gerekebilir."
                )
            if app_target == "desktop" and not resolved_name:
                return f"WhatsApp Desktop aÃ§Ä±lÄ±rken hata oldu: {detail}"
        ...
        if resolved_name:
            ...
            if app_target == "desktop":
                return (
                    "WhatsApp Desktop kiÅŸi adÄ±na gÃ¶re aÃ§Ä±lÄ±rken hata oldu. "
                    f"{detail}. EriÅŸilebilirlik izni ve WhatsApp iÃ§indeki kiÅŸi adÄ± eÅŸleÅŸmesi gerekli olabilir."
                )

    if not normalized_phone:
        if resolved_name:
            return (
                f"'{resolved_name}' iÃ§in kayÄ±tlÄ± bir telefon numarasÄ± bulamadÄ±m ve Desktop aramasÄ± da tamamlanamadÄ±. "
                "Ä°stersen Ã¶nce kiÅŸiyi numarasÄ±yla kaydet."
            )
        return "WhatsApp mesajÄ± iÃ§in kiÅŸi adÄ± veya telefon numarasÄ± gerekli."
    ...
    if not send_now:
        source_note = " (rehberden bulundu)" if contact_source == "phone_book" else ""
        return (
            f"WhatsApp sohbeti {detail} iÃ§inde {resolved_name or f'+{normalized_phone}'}{source_note} iÃ§in taslak mesajla aÃ§Ä±ldÄ±. "
            "GÃ¶ndermek iÃ§in Enter'a bas."
        )

    ok_send, send_detail = _auto_send_with_enter(detail if detail in PREFERRED_BROWSERS else "Google Chrome")
    if ok_send:
        label = resolved_name or f"+{normalized_phone}"
        source_note = " (rehberden bulundu)" if contact_source == "phone_book" else ""
        return f"WhatsApp Web Ã¼zerinden {label}{source_note} kiÅŸisine mesaj gÃ¶nderildi."

    return (
        "WhatsApp Web sohbeti aÃ§Ä±ldÄ± ama otomatik gÃ¶nderim tamamlanamadÄ±. "
        f"{send_detail}. Sistem AyarlarÄ± > Gizlilik ve GÃ¼venlik > EriÅŸilebilirlik izni gerekebilir."
    )
