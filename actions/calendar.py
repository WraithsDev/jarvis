...
Windows'ta yerel JSON takvim deposu kullanilir.
"""
...
HELPER_BIN_DIR = HELPER_CONTENTS_DIR / "Bin"
HELPER_RESOURCES_DIR = HELPER_CONTENTS_DIR / "Resources"
HELPER_INFO_PLIST = HELPER_CONTENTS_DIR / "Info.plist"
HELPER_BIN = HELPER_BIN_DIR / "jarvis-calendar-helper"
WINDOWS_CALENDAR_FILE = BASE_DIR / "memory" / "windows_calendar.json"
...
def _load_windows_events() -> list[dict]:
    try:
        if WINDOWS_CALENDAR_FILE.exists():
            data = json.loads(WINDOWS_CALENDAR_FILE.read_text(encoding="utf-8"))
...
def _save_windows_events(events: list[dict]) -> None:
    WINDOWS_CALENDAR_FILE.parent.mkdir(parents=True, exist_ok=True)
    WINDOWS_CALENDAR_FILE.write_text(json.dumps(events, indent=2, ensure_ascii=False), encoding="utf-8")
...
def _parse_iso_datetime(value: str) -> dt.datetime:
    ...
    for fmt in ("%Y-%m-%d %H:%M", "%Y-%m-%d %H:%M:%S", "%d.%m.%Y %H:%M", "%Y-%m-%d"):
        try:
            ...
            continue
    raise ValueError("Tarih formati gecersiz. YYYY-MM-DDTHH:MM veya YYYY-MM-DD HH:MM kullan.")
...
def _normalize_query(query: str) -> dict:
    ...
    if "gelecek ay" in q or "Ã¶nÃ¼mÃ¼zdeki ay" in q or "onumuzdeki ay" in q or "next month" in q:
    ...
    day_match = re.search(r"(\d+)\s*(g[uÃ¼]n|gun|day|days)", q)
    if day_match:
    ...
    if any(token in q for token in ("siradaki", "sÄ±radaki", "sonraki", "next")):
        return {
            "helper_mode": "next",
            "payload": None,
            "default_limit": 1,
    ...
    if any(token in q for token in ("ajanda", "agenda", "yaklasan", "yaklaÅŸan", "upcoming")):
        return {
            "helper_mode": "agenda",
            "payload": None,
            "default_limit": 8,
...
def get_calendar_events(query: str = "today", limit: int = 6) -> str:
    ...
    if sys.platform.startswith("win"):
        now = dt.datetime.now()
        today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
        events = _load_windows_events()
        normalized = []
        for event in events:
            try:
                start = _parse_iso_datetime(str(event.get("start_iso", "")))
                end_raw = str(event.get("end_iso", "") or "")
                end = _parse_iso_datetime(end_raw) if end_raw else start + dt.timedelta(hours=1)
            except ValueError:
                continue
            item = {
                ...
                "title": str(event.get("title", "")).strip() or "Adsiz etkinlik",
        ...
        if window["kind"] == "today":
            ...
            normalized = [e for e in normalized if today_start.timestamp() <= e["start_ts"] < end.timestamp()]
        elif window["kind"] == "tomorrow":
            start = today_start + dt.timedelta(days=1)
            end = start + dt.timedelta(days=1)
            normalized = [e for e in normalized if start.timestamp() <= e["start_ts"] < end.timestamp()]
        elif window["kind"] in {"week", "agenda"}:
            end = today_start + dt.timedelta(days=7)
            normalized = [e for e in normalized if e["end_ts"] >= now.timestamp() and e["start_ts"] < end.timestamp()]
        elif window["kind"] == "next":
            normalized = [e for e in normalized if e["end_ts"] >= now.timestamp()]

        if not normalized:
            return window["empty"]
        if window["kind"] == "next":
            return f"Siradaki etkinlik: {_format_event_line(normalized[0], now)}."
...
def add_calendar_event(
    ...
    if sys.platform.startswith("win"):
        try:
            ...
            end = _parse_iso_datetime(end_iso) if end_iso and end_iso.strip() else start + dt.timedelta(hours=1)
        ...
        event = {
            ...
            "calendar": (calendar_name or "").strip() or "JARVIS",
            "all_day": bool(all_day),
        }
        events = _load_windows_events()
        events.append(event)
        _save_windows_events(events)
...
def delete_calendar_event(
    ...
    if sys.platform.startswith("win"):
        ...
        deleted = []
        for event in events:
            matches_title = needle in str(event.get("title", "")).casefold()
            matches_calendar = not calendar_name or calendar_name.casefold() in str(event.get("calendar", "")).casefold()
            matches_start = True
            if start_iso and start_iso.strip():
                try:
                    wanted = _parse_iso_datetime(start_iso).date()
                    current = _parse_iso_datetime(str(event.get("start_iso", ""))).date()
                    matches_start = wanted == current
                except ValueError:
                    matches_start = False
            if matches_title and matches_calendar and matches_start and (delete_all_matches or not deleted):
                deleted.append(event)
            else:
                filtered.append(event)
        if not deleted:
            return "Eslesen takvim etkinligi bulunamadi."
        _save_windows_events(filtered)
        return f"Takvimden silindi: {len(deleted)} etkinlik."
